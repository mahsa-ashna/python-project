...
class User:
    def __init__(self, username, password):
        """
        :param username: user should enter name
        :param password: user should enter password
        """
    ...
    def login():
        ...
        #[:, :] is a way of accessing required values with indexing
        try:
            l = list(df_account[:,0]).index(username)
        except:
            print(Fore.LIGHTRED_EX + "no such username ")
            return
    ...
    def register():
        ...
        if v < 0 or v > 3:
            print(Fore.MAGENTA + "no such role")
        else:
